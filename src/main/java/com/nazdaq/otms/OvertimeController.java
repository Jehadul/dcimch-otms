...
import com.nazdaq.otms.model.Employee;
...
public class OvertimeController {
	...
	public ModelAndView generateOtDtl (@ModelAttribute("overtimeDtlForm") OvertimeDtl otDtl, 
		...
		List<OvertimeDtl> otDtlList = (List<OvertimeDtl>)(Object)
				commonService.getObjectListByTwoColumn("OvertimeDtl", "otYear", otYear, "otMonth", otMonth);
		if(otDtlList != null && otDtlList.size() > 0) {
			...
			Integer maxId = (Integer)commonService.getMaxValueByObjectAndColumn("OvertimeDtl", "id");			
			if(maxId > 0 ) {
				...
				if(res) {
					this.saveNewOtDtl(otYear, otMonth, loginUser);
					model.put("otYear", otYear);
					model.put("otMonth", otMonth);
					return new ModelAndView("otDtlListForm", model);
				} else {
	...
	public ModelAndView otDtlFinalSubmit (OvertimeDtl otDtl, Principal principal, RedirectAttributes attributes) {
	...
	private @ResponseBody String getMonthlyOtDltList(@RequestBody String jesonString, Principal principal) 
		...
		List<OvertimeDtl> otDtlList = (List<OvertimeDtl>)(Object)
				commonService.getObjectListByTwoColumn("OvertimeDtl", "otYear", otDtlBean.getOtYear(), "otMonth", otDtlBean.getOtMonth());
		int i = 1;
	...
	private @ResponseBody String saveMonthlyOtDltList(@RequestBody String jesonString, Principal principal) 
			throws JsonGenerationException, JsonMappingException, Exception {
		String toJson = "";
		String result = "";
		Gson gson = new Gson();
	...
	public ModelAndView otDtlDelete (@ModelAttribute("overtimeDtlForm") OvertimeDtl otDtl, 
		...
		String loginUser = principal.getName();
		try {
			commonService.deleteObjectListByThreeColumn("OvertimeDtl", "otYear", otYear, "otMonth", otMonth, "finalSubmit", "0");
	...
	@SuppressWarnings("unchecked")
	private void saveNewOtDtl(String otYear, String otMonth, String loginUser) {
		List<Employee> employeeList = (List<Employee>)(Object)
				commonService.getObjectListByAnyColumn("Employee", "active", "1");
		OvertimeDtl otDtlModel = null;
		Date now = new Date();
